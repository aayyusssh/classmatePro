...
export default function ChatContent(props) {
  ...
  const startCall = () => {
    ...
    axios({
      ...
      .then(res => {
        console.log(res.data);
        socket.emit('sendchat', props.thread_id, `Quick! Come join me in this call https://localhost:3000/call/${res.data.meeting_slug}`, props.name, 'txt');
        const token = localStorage.getItem('token');
        console.log(token)
        axios({
          method: 'post',
          url: api + 'communication/send_message',
          data: {
            msg_text: `Quick! Come join me in this call https://localhost:3000/call/${res.data.meeting_slug}`,
            thread_id: props.thread_id,
          },
          headers: { Authorization: 'Token ' + token }
        })
